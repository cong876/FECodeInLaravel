...
$chargeInstance = \App\Helper\ChargeCacheHelper::getInstance();
...
@foreach($mainOrders as $mainOrder)
    ...
    @foreach($subOrders as $subOrder)
        ...
        if (!empty($charge_id)) {
            $charge = $chargeInstance->getRegionByChargeId($charge_id);
        ...
        <li class="order" data-item-title="{{$title_json}}"
            ...
            data-order-send-time="{{$time_delivery}}"
            data-order-completed-time="{{$time_finish}}"
            ...
            <table>
                ...
                <tr class="orderFooter">
                    <td colspan="2">
                        ...
                        @else
                            <a href="{{$delivery_url}}&callbackurl=http://www.yeyetech.net/user/MyOrder"
                                    style="color: rgb(253,105,114)">查看物流</a></span>
                        @endif
                        <?php } ?>
