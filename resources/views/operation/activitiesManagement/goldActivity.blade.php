        ...
        <section>
            <ol id="itemList">
                @foreach($items as $item)
                    ...
                    $item_info = ['title' => $title, 'description' => $description, 'gold' => $gold, 'marketPrice' => $market_price,
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        var deleteItem = function (item) {                                                  //删除商品函数
            $.ajax({
                url: "/operator/deleteGoldItem/" + item.data("item").id,
                type: "get",
                beforeSend: function (xhr) {
                    var token = $("meta[name=csrf-token]").attr('content');
                    if (token) {
        ...
        $("#itemList").on("click",".publish", function (event) {
            ...
            var that=this;
            $.ajax({
                url: "/operator/publishGoldItem/"+$(that).parents("li").data("item").id,
                type: "get",
                beforeSend: function (xhr) {
                    var token = $("meta[name=csrf-token]").attr('content');
                    if (token) {
        ...
        $("#itemList").on("click",".cancel_publish", function (event) {
            ...
            $.ajax({
                url: "/operator/cancelPublishGoldItem/"+$(that).parents("li").data("item").id,
                type: "get",
                beforeSend: function (xhr) {
                    var token = $("meta[name=csrf-token]").attr('content');
                    if (token) {
        ...
        $("#activityDetail").on("click", ".editItem", function (event) {                    //编辑商品
            ...
            $("#addItem").find(".itemPrice").val(item.gold);
        ...
        $("#addItem").on("change",".itemTitle",function (event) {                           //限制标题长度
            event.preventDefault();
            var that=this;
        ...
        $("#addItem").on("click",".save",function (event) {                                 //保存商品
            ...
            var item = {
                ...
                gold: $("#addItem").find(".itemPrice").val(),
