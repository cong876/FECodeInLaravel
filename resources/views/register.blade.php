...
?>
<input type="hidden" id="determine" value="{{$value}}" >
<!-- loading页	 -->
	<div id="loading">
		<div class="spinner">
			...
			<form class="pure-form pure-form-aligned">
				<fieldset>
					<legend style="margin-bottom: 1em">
						<select id="country">
							<option value="0">请选择您所在的国家/地区</option>
							<?php $countries = App\Models\Country::get(); ?>
							@foreach($countries as $country)
							<option data-code="{{$country->code}}" value="{{$country->country_id}}">{{$country->name}}</option>
							@endforeach
						</select>
					</legend>
			...
			<form class="pure-form pure-form-aligned">
				<fieldset>
					...
					<legend style="margin-bottom: 1em">
						...
						<input type="email" id="email" placeholder="请输入您的常用邮箱">
						<span class="icon-cross clear"></span>
					</legend>
					<legend style="margin-bottom: 0em">
						<span class="icon-lock"></span>
						<input type="password" id="password" name="password" placeholder="请设置安全密码:用于交易提现"/>
					</legend>
					<legend>
	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			...
			if($("#determine").val()==2){
				$("#country").parent().css({marginBottom:"0em"});
				$("fieldset").prepend($("#country").parent().clone())
				$("#register2").show();
				$("#register1").hide();
				$("#register1").remove();	
				$("#email").parent().remove();
			...
			$("#country").on("change",function(event){
				...
				$("#countryCode").val($("#country").find("option").eq($(this).val()).data("code"));
			})
			...
			$("#sendcaptchas").on("tap",function(event){            //发送验证码操作
				...
				$("#loading").show();
				$.ajax({
					url:"/seller/authMobile",                             //验证用户手机是否注册过的url
					type:"post",
					dataType:"json",
					data:data,
			...
			$("#submitOuter").on("tap",function(event){
				...
				if($("#determine").val()==2){
					...
					if(!register.emailPatt.test($("#email").val())){
						alert("邮箱格式不正确，请重新输入");
						return false;
					};
					data={
...
<script type="text/javascript" charset="utf-8">
	wx.config(<?php echo $js->config(array('onMenuShareAppMessage'), false, true) ?>);
	wx.ready(function(){

		wx.onMenuShareAppMessage({
			title: '٩(❛ัᴗ❛ั⁎) 加入红领巾 ٩(❛ัᴗ❛ั⁎)', // 分享标题
			desc: '我宣誓加入红领巾！并服从大队长的安排！', // 分享描述
			imgUrl: 'http://7xln8l.com2.z0.glb.qiniucdn.com/LOGO.jpg', // 分享图标
			success: function () {
				// 用户确认分享后执行的回调函数
			},
			cancel: function () {
